...
use rustscript_syntax::external_fn::ExternalFnInfo;
...
use rustscript_typeck::registry::TypeRegistry;
...
mod tests {
    use super::*;
    use rustscript_syntax::ast::*;
    use rustscript_syntax::rust_ir::{RustExprKind, RustSelfParam, RustStmt};
    ...
    fn compile_and_emit(source: &str) -> String {
        ...
        rustscript_emit::emit(&ir).source
    }
...
function main() {}"#;
    ...
    fn parse_module(source: &str) -> rustscript_syntax::ast::Module {
        let file_id = rustscript_syntax::source::FileId(0);
        let (module, _) = rustscript_parser::parse(source, file_id);
        module
    }
                    ...
                    RustStmt::Let(let_stmt) => match &let_stmt.init.kind {
                        RustExprKind::IteratorChain { ops, terminal, .. } => {
                            ...
                            assert!(
                                matches!(
                                    terminal,
                                    rustscript_syntax::rust_ir::IteratorTerminal::CollectVec
                                ),
                    ...
                    RustExprKind::IteratorChain { ops, .. } => {
                        assert!(
                            ops.iter().any(|op| matches!(
                                op,
                                rustscript_syntax::rust_ir::IteratorOp::Cloned
                            )),
                ...
                RustStmt::Let(let_stmt) => match &let_stmt.init.kind {
                    RustExprKind::IteratorChain { terminal, .. } => {
                        assert!(
                            matches!(
                                terminal,
                                rustscript_syntax::rust_ir::IteratorTerminal::Fold { .. }
                            ),
                ...
                RustStmt::Let(let_stmt) => match &let_stmt.init.kind {
                    RustExprKind::IteratorChain { terminal, .. } => {
                        assert!(
                            matches!(
                                terminal,
                                rustscript_syntax::rust_ir::IteratorTerminal::Find(..)
                            ),
                ...
                RustStmt::Let(let_stmt) => match &let_stmt.init.kind {
                    RustExprKind::IteratorChain { terminal, .. } => {
                        assert!(
                            matches!(
                                terminal,
                                rustscript_syntax::rust_ir::IteratorTerminal::Any(..)
                            ),
                ...
                RustStmt::Let(let_stmt) => match &let_stmt.init.kind {
                    RustExprKind::IteratorChain { terminal, .. } => {
                        assert!(
                            matches!(
                                terminal,
                                rustscript_syntax::rust_ir::IteratorTerminal::All(..)
                            ),
                    ...
                    RustStmt::Semi(expr) => match &expr.kind {
                        RustExprKind::IteratorChain { terminal, .. } => {
                            assert!(
                                matches!(
                                    terminal,
                                    rustscript_syntax::rust_ir::IteratorTerminal::ForEach(..)
                                ),
                ...
                RustStmt::Let(let_stmt) => match &let_stmt.init.kind {
                    RustExprKind::IteratorChain { ops, terminal, .. } => {
                        ...
                        let has_map = ops.iter().any(|op| {
                            matches!(op, rustscript_syntax::rust_ir::IteratorOp::Map(..))
                        });
                        let has_filter = ops.iter().any(|op| {
                            matches!(op, rustscript_syntax::rust_ir::IteratorOp::Filter(..))
                        });
...
async function main() {
    ...
    fn test_lower_external_fn_str_ref_param_strips_to_string() {
        use rustscript_syntax::external_fn::{
            ExternalFnInfo, ExternalParamInfo, ExternalReturnType,
        };
    ...
    fn test_lower_external_fn_throws_adds_question_mark() {
        use rustscript_syntax::external_fn::{ExternalFnInfo, ExternalReturnType};
...
type Bad = Pick<User, "nonexistent">"#;
    ...
    fn lower_source_with_diagnostics(
        source: &str,
    ) -> (RustFile, Vec<rustscript_syntax::diagnostic::Diagnostic>) {
        let file_id = rustscript_syntax::source::FileId(0);
        let (module, parse_diags) = rustscript_parser::parse(source, file_id);
        assert!(
            parse_diags.is_empty(),
