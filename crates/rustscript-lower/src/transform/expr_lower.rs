...
impl Transform {
    ...
    pub(super) fn lower_expr(
        ...
        match &expr.kind {
            ...
            ast::ExprKind::Super => {
                ...
                ctx.emit_diagnostic(rustscript_syntax::diagnostic::Diagnostic::error(
            ...
            ast::ExprKind::Cast(inner, type_ann) => {
                ...
                let ty = rustscript_typeck::resolve::resolve_type_annotation_to_rust_type(
                    type_ann, &mut diags,
                );
                for d in diags {
                ...
                if let ast::ExprKind::Ident(ident) = &operand.kind {
                    RustExpr::new(
                        RustExprKind::CompoundAssign {
                            ...
                            op: rustscript_syntax::rust_ir::RustCompoundAssignOp::AddAssign,
            ...
            ast::ExprKind::PostfixDecrement(operand) | ast::ExprKind::PrefixDecrement(operand) => {
                if let ast::ExprKind::Ident(ident) = &operand.kind {
                    RustExpr::new(
                        RustExprKind::CompoundAssign {
                            target: ident.name.clone(),
                            op: rustscript_syntax::rust_ir::RustCompoundAssignOp::SubAssign,
                            value: Box::new(RustExpr::new(RustExprKind::IntLit(1), expr.span)),
                        },
                        expr.span,
                    )
    ...
    fn lower_call_expr(
        &self,
        call: &ast::CallExpr,
        span: rustscript_syntax::span::Span,
        ctx: &mut LoweringContext,
        use_map: &UseMap,
        stmt_index: usize,
    ) -> RustExpr {
    ...
    fn lower_single_arg(
        ...
        if let ast::ExprKind::StringLit(s) = &a.kind
            ...
            && matches!(
                &td.kind,
                rustscript_typeck::registry::TypeDefKind::SimpleEnum(_)
                    | rustscript_typeck::registry::TypeDefKind::DataEnum(_)
            )
        {
            return RustExpr::new(
    ...
    fn lower_closure(
        ...
        let params: Vec<RustClosureParam> = closure
            ...
            .map(|p| {
                ...
                let ty = rustscript_typeck::resolve::resolve_type_annotation_with_generics(
                    &p.type_ann,
                    &self.type_registry,
                    &[],
                    &mut diags,
    ...
    fn lower_method_call(
        ...
        if mc.method.name == "lock"
            ...
            && matches!(
                var_info.ty,
                rustscript_syntax::rust_ir::RustType::ArcMutex(_)
            )
        {
            let receiver = self.lower_expr(&mc.object, ctx, use_map, stmt_index);
            // .lock() → .lock().unwrap()
    ...
    fn lower_new_expr(
        ...
        let rust_type_name =
            rustscript_typeck::resolve::map_collection_type_name(&new_expr.type_name.name);
    ...
    fn build_regex_pattern_arg(
        raw_args: &[rustscript_syntax::ast::Expr],
        ...
        let flags_str = raw_args.get(1).and_then(|expr| {
            if let rustscript_syntax::ast::ExprKind::StringLit(s) = &expr.kind {
                Some(s.as_str())
            } else {
                None
            }
