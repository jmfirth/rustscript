...
impl Transform {
    ...
    pub(super) fn register_class_def(&mut self, cls: &ast::ClassDef, ctx: &mut LoweringContext) {
        ...
        let mut fields: Vec<(String, rustscript_typeck::types::Type)> = cls
            .members
            .iter()
            ...
            if let ast::ClassMember::Constructor(ctor) = member {
                ...
                if has_defaults {
                    ...
                    let param_types: Vec<RustType> = ctor
                        ...
                        .map(|p| {
                            let ty =
                                rustscript_typeck::resolve::resolve_type_annotation_with_generics(
                                    &p.type_ann,
                                    &self.type_registry,
                                    &generic_names,
                                    &mut temp_diags,
    ...
    pub(super) fn lower_class_def(
            ...
            && let Some(base_fields) = self.type_registry.get_class_fields(&base_name.name)
        {
            for (fname, fty) in base_fields {
                let rust_ty = rustscript_typeck::bridge::type_to_rust_type(fty);
    ...
    fn is_concrete_base_class(&self, name: &str) -> bool {
        self.extended_classes.contains(name)
            && self.type_registry.lookup(name).is_some_and(|td| {
                matches!(
                    td.kind,
                    rustscript_typeck::registry::TypeDefKind::Class { .. }
                )
            })
    }
    ...
    fn generate_inheritance_trait(
        ...
        cls: &ast::ClassDef,
        exported: bool,
        type_params: &[rustscript_syntax::rust_ir::RustTypeParam],
        ctx: &mut LoweringContext,
    ) -> Vec<RustItem> {
    ...
    fn lower_class_constructor(
        ...
        ctx: &mut LoweringContext,
        diags: &mut Vec<rustscript_syntax::diagnostic::Diagnostic>,
    ) -> RustMethod {
        ctx.push_scope();
        ...
        let mut other_stmts: Vec<rustscript_syntax::rust_ir::RustStmt> = Vec::new();
        ...
        other_stmts.push(rustscript_syntax::rust_ir::RustStmt::Expr(
            RustExpr::synthetic(RustExprKind::SelfStructLit {
                fields: self_fields,
            }),
        ));
