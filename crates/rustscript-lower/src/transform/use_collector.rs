...
fn scan_expr_for_collections(expr: &RustExpr, needs_hashmap: &mut bool, needs_hashset: &mut bool) {
    match &expr.kind {
        ...
        RustExprKind::SpreadArray { initial, ops } => {
            ...
            for op in ops {
                match op {
                    rustscript_syntax::rust_ir::SpreadOp::Push(expr)
                    | rustscript_syntax::rust_ir::SpreadOp::Extend(expr) => {
                        scan_expr_for_collections(expr, needs_hashmap, needs_hashset);
                    }
                }
            }
        ...
        RustExprKind::IteratorChain {
            ...
            for op in ops {
                match op {
                    rustscript_syntax::rust_ir::IteratorOp::Map(_, body)
                    | rustscript_syntax::rust_ir::IteratorOp::Filter(_, body) => {
                        scan_expr_for_collections(body, needs_hashmap, needs_hashset);
                    }
                    rustscript_syntax::rust_ir::IteratorOp::MapFnRef(fn_expr)
                    | rustscript_syntax::rust_ir::IteratorOp::FilterFnRef(fn_expr) => {
                        scan_expr_for_collections(fn_expr, needs_hashmap, needs_hashset);
                    }
                    rustscript_syntax::rust_ir::IteratorOp::Cloned
                    | rustscript_syntax::rust_ir::IteratorOp::Rev => {}
                }
            }
            match terminal {
                rustscript_syntax::rust_ir::IteratorTerminal::CollectVec => {}
                rustscript_syntax::rust_ir::IteratorTerminal::Fold { init, body, .. } => {
                ...
                rustscript_syntax::rust_ir::IteratorTerminal::Find(_, body)
                | rustscript_syntax::rust_ir::IteratorTerminal::Any(_, body)
                | rustscript_syntax::rust_ir::IteratorTerminal::All(_, body)
                | rustscript_syntax::rust_ir::IteratorTerminal::ForEach(_, body) => {
                    scan_expr_for_collections(body, needs_hashmap, needs_hashset);
                }
            }
        }
    ...
    fn test_use_collector_detects_box_dyn_any_in_fn_param() {
        let items = vec![RustItem::Function(RustFnDecl {
            ...
            params: vec![RustParam {
                ...
                mode: rustscript_syntax::rust_ir::ParamMode::Owned,
                span: None,
            }],
            return_type: None,
            body: RustBlock {
