...
impl Transform {
    ...
    fn lower_var_decl(
        ...
        } else if let Some(lit_ty) = rustscript_typeck::resolve::infer_literal_rust_type(&decl.init)
        {
            lit_ty
        ...
        let type_inferred_from_literal = !has_explicit_annotation
            && rustscript_typeck::resolve::infer_literal_rust_type(&decl.init).is_some();
    ...
    fn lower_if(
        ...
        let else_clause = if_stmt.else_clause.as_ref().map(|ec| match ec {
            ast::ElseClause::Block(block) => rustscript_syntax::rust_ir::RustElse::Block(
                self.lower_block(block, ctx, use_map, stmt_index, reassigned),
            ),
            ast::ElseClause::ElseIf(nested_if) => rustscript_syntax::rust_ir::RustElse::ElseIf(
                Box::new(self.lower_if(nested_if, ctx, use_map, stmt_index, reassigned)),
            ),
        });
