...
//!
//! Every test in this module calls `rustscript_parser::parse` with pathological input
...
/// Run `rustscript_parser::parse` on `input` and assert it neither panics nor hangs.
///
...
fn assert_parse_survives(input: &str) {
    ...
    thread::spawn(move || {
        let result = panic::catch_unwind(panic::AssertUnwindSafe(|| {
            let _ = rustscript_parser::parse(&input, FILE_ID);
        }));
