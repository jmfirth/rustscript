...
//!
//! Consumes the token stream from the lexer and produces a [`rustscript_syntax::ast::Module`].
...
impl<'src> Parser<'src> {
    ...
    fn parse_constructor_param_list(&mut self) -> Vec<ConstructorParam> {
        use rustscript_syntax::ast::ConstructorParam;
  ...
  for (let i = 0; i < 20; i += 2) {}
    ...
    fn test_for_in_switch_case() {
        ...
        if let Stmt::Switch(switch) = stmt {
            ...
            assert_eq!(
                switch.cases[0].pattern,
                SwitchPattern::StringLit("loop".to_string())
            );
