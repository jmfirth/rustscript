...
/// Lower `console.timeLog(label)` to `eprintln!("{}: timer log", label)`.
///
/// True timing requires runtime state tracking. This simplified version
/// prints the label to stderr to indicate a timer log entry.
fn lower_console_time_log(args: Vec<RustExpr>, span: Span) -> RustExpr {
    let label = args
        ...
        .unwrap_or_else(|| RustExpr::synthetic(RustExprKind::StringLit("default".into())));
    let macro_args = vec![
        RustExpr::synthetic(RustExprKind::StringLit("{}: timer log".into())),
        strip_to_string(label),
    ];
    RustExpr::new(
        RustExprKind::Macro {
...
mod tests {
    ...
    #[test]
    fn test_lower_console_time_log_produces_eprintln() {
        let result = lower_console_time_log(vec![string_arg("benchmark")], span());
        match &result.kind {
            RustExprKind::Macro { name, args } => {
                assert_eq!(name, "eprintln");
                assert!(
                    matches!(&args[0].kind, RustExprKind::StringLit(s) if s.contains("timer log"))
                );
            }
            other => panic!("expected Macro(eprintln), got {other:?}"),
        }
