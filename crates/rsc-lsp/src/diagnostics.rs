...
//!
//! Translates [`rsc_syntax::diagnostic::Diagnostic`] to [`tower_lsp::lsp_types::Diagnostic`],
...
pub fn collect_diagnostics(source: &str) -> Vec<lsp_types::Diagnostic> {
    ...
    let (_, parse_diags) = rsc_parser::parse(source, file_id);
    ...
    let result = rsc_driver::compile_source(source, "lsp_input.rts");
