...
//! parsed AST. Used by both the WASM playground (`rsc-web`) and the VS Code
//! LSP (`rsc-lsp`).
...
fn collect_fn_info(module: &rsc_syntax::ast::Module) -> HashMap<String, FnInfo> {
    use rsc_syntax::ast::ItemKind;
...
#[allow(dead_code)]
fn collect_fn_return_types(module: &rsc_syntax::ast::Module) -> HashMap<String, String> {
...
fn collect_var_types(stmts: &[rsc_syntax::ast::Stmt]) -> HashMap<String, String> {
    use rsc_syntax::ast::Stmt;
    let mut map = HashMap::new();
    for stmt in stmts {
...
fn collect_type_fields(module: &rsc_syntax::ast::Module) -> HashMap<String, Vec<(String, String)>> {
    use rsc_syntax::ast::ItemKind;
    let mut map = HashMap::new();
    for item in &module.items {
...
fn infer_type_from_expr_ctx(expr: &rsc_syntax::ast::Expr, ctx: &InferCtx<'_>) -> Option<String> {
    use rsc_syntax::ast::ExprKind;
...
#[must_use]
pub fn format_type_ann(ty: &rsc_syntax::ast::TypeAnnotation) -> String {
