...
use rustscript_driver::rustdoc_parser::RustdocItemKind;
use rustscript_syntax::diagnostic::Severity;
use rustscript_syntax::source::compute_line_starts;
...
fn build_diagnostics(
    diagnostics: &[rustscript_syntax::diagnostic::Diagnostic],
    source: &str,
...
pub fn compile(source: &str) -> JsValue {
    let result = rustscript_driver::compile_source(source, "playground.rts");
...
pub fn get_diagnostics(source: &str) -> JsValue {
    ...
    let (module, parse_diagnostics) = rustscript_parser::parse(source, file_id);
...
///
/// Delegates to the shared `rustscript-hover` crate which contains all hover logic.
#[wasm_bindgen]
#[allow(clippy::must_use_candidate)]
pub fn hover(source: &str, line: u32, column: u32) -> String {
    rustscript_hover::hover(source, line, column)
}
...
pub fn translate_rustdoc(json: &str) -> JsValue {
    ...
    let Some(crate_data) = rustscript_driver::rustdoc_parser::parse_rustdoc_json(&parsed) else {
        return JsValue::NULL;
    };
...
mod tests {
    ...
    fn test_compile_hello_world() {
        ...
        let result = rustscript_driver::compile_source(source, "test.rts");
        assert!(
            !result.has_errors,
    ...
    fn test_build_diagnostics_line_column() {
        ...
        let diag = rustscript_syntax::diagnostic::Diagnostic::error("test error").with_label(
            rustscript_syntax::span::Span::new(20, 21),
            rustscript_syntax::source::FileId(0),
            "here",
        );
    ...
    fn test_translate_rustdoc_empty_index() {
        ...
        let crate_data = rustscript_driver::rustdoc_parser::parse_rustdoc_json(&parsed);
