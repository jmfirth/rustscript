...
[workspace]\n";
    ...
    pub fn eval(&mut self, input: &str) -> Result<Option<String>> {
        ...
        let result = rustscript_driver::compile_source(&rts_source, "repl.rts");

        if result.has_errors {
            // Render diagnostics to a string
            let mut buf = Vec::new();
            let _ = rustscript_syntax::diagnostic::render_diagnostics(
                &result.diagnostics,
                &result.source_map,
                &mut buf,
            );
