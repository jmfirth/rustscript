...
//!
//! Parses command-line arguments and delegates to `rustscript-driver` for all
...
use rustscript_driver::ColorMode;
use rustscript_driver::error::DriverError;
use rustscript_driver::{Project, compile_source, init_project};
...
#[derive(Parser)]
#[command(name = "rustscript", about = "The RustScript compiler", version)]
struct Cli {
    /// Color output mode
...
fn cmd_check(color: ColorMode) -> Result<i32> {
    ...
    if result.has_errors {
        ...
        let _ = rustscript_syntax::diagnostic::render_diagnostics_colored(
            &result.diagnostics,
            &result.source_map,
            &mut stderr,
            color,
...
fn cmd_fmt(check: bool, files: &[PathBuf]) -> Result<i32> {
    ...
    for path in &sources {
        ...
        let formatted = rustscript_fmt::format_source(&source)
...
fn cmd_add(crate_name: &str, version: Option<&str>, features: &[String], dev: bool) -> Result<i32> {
    let project = open_project()?;
    match rustscript_driver::deps::add_dependency(&project.root, crate_name, version, features, dev)
    {
        Ok(result) => {
...
fn cmd_remove(crate_name: &str) -> Result<i32> {
    let project = open_project()?;
    match rustscript_driver::deps::remove_dependency(&project.root, crate_name) {
        Ok(()) => {
            println!("Removed {crate_name} from rustscript.json");
            Ok(0)
        }
        Err(rustscript_driver::error::DriverError::DependencyNotFound(_)) => {
...
fn cmd_lsp() -> Result<i32> {
    rustscript_lsp::run_server().map_err(|e| anyhow::anyhow!("LSP server failed: {e}"))?;
    Ok(0)
}
...
fn emit_types_to_dir(project_root: &Path, out: &Path) -> Result<()> {
    let results = rustscript_driver::typegen::generate_types_for_project(project_root)
