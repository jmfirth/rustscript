...
use rustscript_driver::rustdoc_cache::RustdocCache;
use rustscript_driver::rustdoc_parser;
...
impl RscLanguageServer {
    ...
    async fn publish_diagnostics(&self, uri: &Url, source: &str) {
        ...
        let (module, parse_diags) = rustscript_parser::parse(source, file_id);
        let has_parse_errors = parse_diags
            .iter()
            .any(|d| matches!(d.severity, rustscript_syntax::diagnostic::Severity::Error));
        ...
        let compile_result = rustscript_driver::compile_source(source, "lsp_input.rts");
    ...
    fn schedule_diagnostics(&self, uri: Url) {
        ...
        tokio::spawn(async move {
            ...
            if let Some(source) = documents.get(&uri) {
                ...
                let file_id = rustscript_syntax::source::FileId(0);
                let (module, parse_diags) = rustscript_parser::parse(&source_text, file_id);
                let has_parse_errors = parse_diags
                    .iter()
                    .any(|d| matches!(d.severity, rustscript_syntax::diagnostic::Severity::Error));
                ...
                let compile_result =
                    rustscript_driver::compile_source(&source_text, "lsp_input.rts");
                if !compile_result.has_errors {
                    let position_map = PositionMap::new(
                        compile_result.source_map_lines,
                        source_text,
...
impl LanguageServer for RscLanguageServer {
    ...
    async fn formatting(&self, params: DocumentFormattingParams) -> Result<Option<Vec<TextEdit>>> {
        ...
        if let Some(source) = self.documents.get(uri) {
            ...
            match rustscript_fmt::format_source(&source_text) {
    ...
    async fn hover(&self, params: HoverParams) -> Result<Option<Hover>> {
        ...
        if let Some(source) = self.documents.get(uri) {
            ...
            let file_id = rustscript_syntax::source::FileId(0);
            let (module, _) = rustscript_parser::parse(&source_text, file_id);
            ...
            // Fall back to rustscript-hover's token-based hover for builtins and
            // user-defined symbols that the span-based system missed.
            // rustscript-hover expects 1-based line/column (Monaco convention);
            // LSP uses 0-based.
            let hover_result =
                rustscript_hover::hover(&source_text, position.line + 1, position.character + 1);
...
fn find_hover_info(
    ...
    use rustscript_syntax::span::BytePos;
...
fn stmt_span(stmt: &rustscript_syntax::ast::Stmt) -> rustscript_syntax::span::Span {
...
fn find_hover_in_stmts(
    ...
    pos: rustscript_syntax::span::BytePos,
    cache: Option<&CachedCompileInfo>,
) -> Option<String> {
...
fn format_function_signature(func: &rustscript_syntax::ast::FnDecl) -> String {
...
fn format_param(param: &rustscript_syntax::ast::Param) -> String {
...
fn format_type_params(type_params: Option<&rustscript_syntax::ast::TypeParams>) -> String {
    match type_params {
...
fn format_typedef_hover(td: &rustscript_syntax::ast::TypeDef) -> String {
...
fn format_enum_hover(ed: &rustscript_syntax::ast::EnumDef) -> String {
    use rustscript_syntax::ast::EnumVariant;
...
fn format_interface_hover(iface: &rustscript_syntax::ast::InterfaceDef) -> String {
...
fn format_class_hover(class: &rustscript_syntax::ast::ClassDef) -> String {
...
fn format_type(type_ann: &rustscript_syntax::ast::TypeAnnotation) -> String {
    ...
    match &type_ann.kind {
        ...
        TypeKind::MappedType {
            ...
            let opt = match optional {
                Some(rustscript_syntax::ast::MappedModifier::Add) => "?",
                Some(rustscript_syntax::ast::MappedModifier::Remove) => "-?",
                None => "",
            };
            format!(
                "{{ {ro}[{} in {}]{opt}: {} }}",
    ...
    match &expr.kind {
        ...
        ExprKind::LogicalAssign(la) => {
            ...
            let op_str = match la.op {
                rustscript_syntax::ast::LogicalAssignOp::NullishAssign => "??=",
                rustscript_syntax::ast::LogicalAssignOp::OrAssign => "||=",
                rustscript_syntax::ast::LogicalAssignOp::AndAssign => "&&=",
            };
        ...
        ExprKind::ArrayLit(elems) => {
            for elem in elems {
                let inner = match elem {
                    rustscript_syntax::ast::ArrayElement::Expr(e)
                    | rustscript_syntax::ast::ArrayElement::Spread(e) => e,
                };
                if let Some(info) = find_hover_in_expr(inner, pos, cache) {
                    return Some(info);
                }
        ...
        ExprKind::TemplateLit(tl) => {
            for part in &tl.parts {
                if let rustscript_syntax::ast::TemplatePart::Expr(expr) = part
...
fn find_hover_in_class_members(
    class: &rustscript_syntax::ast::ClassDef,
    pos: rustscript_syntax::span::BytePos,
    cache: Option<&CachedCompileInfo>,
) -> Option<String> {
    use rustscript_syntax::ast::{ClassMember, Visibility};
...
fn format_binary_op_hover(op: rustscript_syntax::ast::BinaryOp) -> String {
    use rustscript_syntax::ast::BinaryOp;
    match op {
...
fn format_unary_op_hover(op: rustscript_syntax::ast::UnaryOp) -> String {
    use rustscript_syntax::ast::UnaryOp;
    match op {
...
fn build_compile_cache(module: &rustscript_syntax::ast::Module) -> CachedCompileInfo {
    let lower_result = rustscript_lower::lower(module);
    let mut variable_types = HashMap::new();
    let mut function_signatures = HashMap::new();

    for item in &lower_result.ir.items {
        match item {
            rustscript_syntax::rust_ir::RustItem::Function(func) => {
            ...
            rustscript_syntax::rust_ir::RustItem::Struct(s) => {
    ...
    for item in &module.items {
        if let rustscript_syntax::ast::ItemKind::Function(func) = &item.kind {
...
fn infer_literal_type_string(kind: &rustscript_syntax::ast::ExprKind) -> Option<String> {
    use rustscript_syntax::ast::ExprKind;
    match kind {
...
fn format_rust_function_sig(func: &rustscript_syntax::rust_ir::RustFnDecl) -> String {
    let mut sig = String::new();
    if func.is_async {
        sig.push_str("async ");
    }
...
fn rust_type_to_rts_type(ty: &rustscript_syntax::rust_ir::RustType) -> String {
...
fn extract_var_types_from_stmts(
    stmts: &[rustscript_syntax::rust_ir::RustStmt],
    types: &mut HashMap<String, String>,
) {
    use rustscript_syntax::rust_ir::{RustElse, RustStmt};
    for stmt in stmts {
        match stmt {
...
mod tests {
    ...
    fn test_server_hover_function_name_returns_signature() {
        ...
        let file_id = rustscript_syntax::source::FileId(0);
        let (module, _) = rustscript_parser::parse(source, file_id);
    ...
    fn test_server_correctness_format_unformatted_source_returns_edit() {
        ...
        let formatted = rustscript_fmt::format_source(source).unwrap();
    ...
    fn test_server_format_type_params_single() {
        use rustscript_syntax::ast::{Ident, TypeParam, TypeParams};
        use rustscript_syntax::span::Span;
    ...
    fn test_server_format_type_params_with_constraint() {
        use rustscript_syntax::ast::{Ident, TypeAnnotation, TypeKind, TypeParam, TypeParams};
        use rustscript_syntax::span::Span;
