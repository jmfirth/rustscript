...
fn test_e2e_p3_template_format_build_pipeline() {
    ...
    // Format it
    let formatted = rustscript_fmt::format_source(&original).expect("format should succeed");
...
fn test_e2e_p3_format_compile_roundtrip_async() {
  ...
  return `Hello, ${name}!`;
    ...
    // Format
    let formatted = rustscript_fmt::format_source(source).expect("format should succeed");
...
fn test_e2e_p3_error_translation_integration() {
    ...
    let source_map = vec![
        Some(rustscript_syntax::span::Span::new(0, 10)),
        Some(rustscript_syntax::span::Span::new(11, 30)),
    ];
    let rts_source = "function main() {\n  const x: string = 42;\n}\n";

    let translated = rustscript_driver::translate_rustc_errors(
        rustc_stderr,
        Some(&source_map),
        Some(rts_source),
        Some("src/main.rts"),
