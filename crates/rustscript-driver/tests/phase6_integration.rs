...
function main() {
  console.log(greet(\"world\"));
    ...
    let options = rustscript_driver::CompileOptions {
        no_borrow_inference: true,
        ..rustscript_driver::CompileOptions::default()
    };
    let result = rustscript_driver::compile_source_with_options(source, "test.rts", &options);
    assert!(
        !result.has_errors,
...
fn test_dynamic_import_emits_diagnostic() {
    let result = rustscript_driver::compile_source(
