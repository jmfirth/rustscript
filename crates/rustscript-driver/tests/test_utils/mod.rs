...
///
/// Panics if compilation produces errors.
pub fn compile_result(rts_source: &str) -> rustscript_driver::CompileResult {
    let result = compile_source(rts_source, "test.rts");
...
pub fn compile_multi_file_and_run(files: &[(&str, &str)]) -> String {
    ...
    let project = rustscript_driver::Project::open(&project_dir).expect("failed to open project");
