...
pub struct CompileResult {
    ...
    pub crate_dependencies: Vec<rustscript_lower::CrateDependency>,
...
pub fn compile_source_with_options(
    ...
    // Stage 2: Lower
    let lower_options = rustscript_lower::LowerOptions {
        no_borrow_inference: options.no_borrow_inference,
        external_signatures: options.external_signatures.clone(),
    };
    let lower_result = rustscript_lower::lower_with_options(&module, &lower_options);
    ...
    // Stage 3: Emit
    let emit_result = rustscript_emit::emit(&lower_result.ir);
...
pub fn compile_source_with_mods(
    source: &str,
    file_name: &str,
    mod_decls: Vec<rustscript_syntax::rust_ir::RustModDecl>,
) -> CompileResult {
...
pub fn compile_source_with_mods_and_options(
    ...
    // Stage 3: Emit
    let emit_result = rustscript_emit::emit(&ir);
