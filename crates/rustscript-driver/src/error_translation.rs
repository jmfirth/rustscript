...
use regex::Regex;
use rustscript_syntax::diagnostic::ColorMode;
...
fn synthesized_code_annotation(
    ...
    if let Some((_idx, span)) = nearest {
        if let Some(source) = rts_source {
            ...
            Some("This error is in code generated by the RustScript compiler.".to_owned())
        }
    } else {
...
"#;
    ...
    fn test_json_rendering_applies_source_map() {
        use rustscript_syntax::span::{BytePos, Span};
    ...
    fn test_enrich_moved_value() {
        ...
        assert!(hint.contains("clone"), "hint should suggest clone: {hint}");
    }
    ...
    fn test_enrich_lifetime() {
        ...
        let hint = hint.unwrap();
        assert!(hint.contains("scope"), "hint should mention scope: {hint}");
        assert!(
    ...
    fn test_enrich_not_found() {
        ...
        let hint = hint.unwrap();
        assert!(hint.contains("typos"), "hint should mention typos: {hint}");
        assert!(
    ...
    fn test_enrichment_synthesized_annotation_in_output() {
        ...
        let source_map: Vec<Option<Span>> = vec![Some(Span::new(0, 5)), None, None];
        let rts_source = "line1\nline2\nline3\n";
